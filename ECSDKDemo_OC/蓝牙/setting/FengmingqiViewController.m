...
@interface FengmingqiViewController ()
{
...
@implementation FengmingqiViewController
...
- (void)layoutNavigation {
    ...
    labelcenter.text = LocationLanguage(@"buzzer", @"蜂鸣器");//CustomLocalizedString(@"xuanzeyuyan", nil);
...
- (void)getInitData
{
    if(![BLEManager sharedManager].isConnecting){
        ...
        NSString *strHeightCmdCode_strHexData = [NSString stringWithFormat:@"%@",@"PR020101"];
...
- (void)viewDidLoad
{
 ...
 if( ([[[UIDevice currentDevice] systemVersion] doubleValue]>=7.0))
    ...
    [super viewDidLoad];
    self.title = LocationLanguage(@"buzzer", @"蜂鸣器");
...
//    //    m_tableView.tableHeaderView = tableHeaderView;
    ...
    dataArr = [NSMutableArray arrayWithObjects:LocationLanguage(@"open", @"开启"),
               LocationLanguage(@"off", @"关闭"),  nil];
    MyTable = [SingleChooseTable ShareTableWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, kDeviceHeight - 64)];
    MyTable.dataArr = dataArr;
    
//    NSString *fengmingqi =[[NSUserDefaults standardUserDefaults] objectForKey:@"fengmingqi"];
//    if(fengmingqi !=NULL){
//        MyTable.chooseContent=fengmingqi;
//    }
//
//    [MyTable ReloadData];
    //选中内容
    MyTable.block = ^(NSString *chooseContent,NSIndexPath *indexPath){
        ...
        [MyTable ReloadData];
        [[NSUserDefaults standardUserDefaults] setObject:chooseContent forKey:@"fengmingqi"];
        ...
        NSString *strcmd = [HexUtils getisBuzzerOpenedCmd:@"0000"];
        if(indexPath.row==0)
        {
          strcmd = [HexUtils getisBuzzerOpenedCmd:@"0001"];
        }
        else if(indexPath.row==1)
        {
            strcmd = [HexUtils getisBuzzerOpenedCmd:@"0000"];
        }
...
- (void)BLEManagerReceiveData:(NSData *)value fromPeripheral:(CBPeripheral *)peripheral andServiceUUID:(NSString *)serviceUUID andCharacteristicUUID:(NSString *)charUUID
{
    ...
    if ([str_receve containsString:@"1B41"]) {
        if(str_receve.length>12)
        {
            ...
            {
                MyTable.chooseContent= LocationLanguage(@"open", @"开启");
                [MyTable ReloadData];
            }
            else
            {
                MyTable.chooseContent= LocationLanguage(@"off", @"关闭");
                [MyTable ReloadData];
            }
