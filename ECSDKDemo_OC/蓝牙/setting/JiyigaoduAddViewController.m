...
@implementation JiyigaoduAddViewController
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self)
	{
		self.title = LocationLanguage(@"add", @"高度添加");
...
- (void)RightBarButtonItemPressed {
    ...
    if(str_name.length > 10) {
        [SVProgressHUD showErrorWithStatus:LocationLanguage(@"pleaseNameMemoryHeight", @"请输入1~10个字符的记忆高度名称")];
        [txtName becomeFirstResponder];
        return;
    }
...
//    NSString * intriduction = self.textField.text;
    for (NSInteger i = 0; i < specialStringArray.count; i ++) {
        //判断字符串中是否含有特殊符号
        if ([str_name rangeOfString:specialStringArray[i]].location != NSNotFound) {
    ...
    if(data_gaodu.count >= 12) {
        [SVProgressHUD showErrorWithStatus:LocationLanguage(@"max12", @"最多可设置12组记忆高度数据")];
        [txtName becomeFirstResponder];
        return;
    }
    for (int i=0; i< data_gaodu.count; i++) {
        Gaodu *p4 = data_gaodu[i];
        if([str_name isEqualToString:p4.name]){
            [SVProgressHUD showErrorWithStatus:LocationLanguage(@"nameExists", @"高度名称不能重名")];
            [txtName becomeFirstResponder];
            return;
        }
    }
