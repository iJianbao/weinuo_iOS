...
@interface PengzhuangbaohuViewController ()
{
...
@implementation PengzhuangbaohuViewController
...
//    labelcenter.text = CustomLocalizedString(@"xuanzeyuyan", nil);
...
- (void)getInitData
{
    if(![BLEManager sharedManager].isConnecting){
        ...
        NSString *strHeightCmdCode_strHexData = [NSString stringWithFormat:@"%@",@"PR020401"];
...
- (void)viewDidLoad
{
 ...
 if( ([[[UIDevice currentDevice] systemVersion] doubleValue]>=7.0))
    ...
    [super viewDidLoad];
    self.title = LocationLanguage(@"protection", @"碰撞保护");
...
//    //    m_tableView.tableHeaderView = tableHeaderView;
    ...
    dataArr = [NSMutableArray arrayWithObjects:LocationLanguage(@"close", @"关闭"),
               LocationLanguage(@"mild", @"轻度"),
               LocationLanguage(@"moderate", @"中度"),
               LocationLanguage(@"severe", @"重度"),  nil];
    MyTable = [SingleChooseTable ShareTableWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, kDeviceHeight)];
    MyTable.dataArr = dataArr;
    

    
        NSString *pengzhuangbaohu =[[NSUserDefaults standardUserDefaults] objectForKey:@"pengzhuangbaohu"];
        if(pengzhuangbaohu !=NULL){
            MyTable.chooseContent=pengzhuangbaohu;
        }
    ...
    MyTable.block = ^(NSString *chooseContent,NSIndexPath *indexPath){
        NSLog(@"数据：%@ ；第%ld行",chooseContent,indexPath.row);
         ...
         [MyTable ReloadData];
        [[NSUserDefaults standardUserDefaults] setObject:chooseContent forKey:@"pengzhuangbaohu"];
        ...
        NSString *strcmd = [HexUtils getCrashDefendCmd:@"0000"];
        if(indexPath.row==0)
        {
            strcmd = [HexUtils getCrashDefendCmd:@"0000"];
        }
        else if(indexPath.row==1)
        {
            strcmd = [HexUtils getCrashDefendCmd:@"0001"];
        }
        else if(indexPath.row==2)
        {
            strcmd = [HexUtils getCrashDefendCmd:@"0002"];
        }
        else if(indexPath.row==3)
        {
            strcmd = [HexUtils getCrashDefendCmd:@"0003"];
        }
...
- (void)BLEManagerReceiveData:(NSData *)value fromPeripheral:(CBPeripheral *)peripheral andServiceUUID:(NSString *)serviceUUID andCharacteristicUUID:(NSString *)charUUID
{
    ...
    if ([str_receve containsString:@"1B41"]) {
        if(str_receve.length > 12)
        {
            ...
            if([str_gaodu isEqualToString:@"0.0"])
            {
                MyTable.chooseContent= LocationLanguage(@"close", @"关闭");
                [MyTable ReloadData];
            }
            else if([str_gaodu isEqualToString:@"1"])
            {
                MyTable.chooseContent= LocationLanguage(@"mild", @"轻度");
                [MyTable ReloadData];
            }
            else if([str_gaodu isEqualToString:@"2"])
            {
                MyTable.chooseContent= LocationLanguage(@"moderate", @"中度");
                [MyTable ReloadData];
            }
            else if([str_gaodu isEqualToString:@"3"])
            {
                MyTable.chooseContent= LocationLanguage(@"severe", @"重度");
                [MyTable ReloadData];
            }
        }
    }
