...
@interface DanweishezhiViewController ()
{
...
@implementation DanweishezhiViewController
...
- (void)layoutNavigation {
    ...
    labelcenter.text = LocationLanguage(@"unit", @"单位设置");// CustomLocalizedString(@"xuanzeyuyan", nil);
...
- (void)viewDidLoad
{
 ...
 if( ([[[UIDevice currentDevice] systemVersion] doubleValue]>=7.0))
    ...
    [super viewDidLoad];
    self.title = LocationLanguage(@"unit", @"单位设置");
...
//    //    m_tableView.tableHeaderView = tableHeaderView;
    ...
    dataArr = [NSMutableArray arrayWithObjects:[NSString stringWithFormat:@"%@(cm)", LocationLanguage(@"cm", @"厘米")],
               [NSString stringWithFormat:@"%@(inch)", LocationLanguage(@"inch", @"厘米")],  nil];
    MyTable = [SingleChooseTable ShareTableWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, kDeviceHeight - 64)];
    MyTable.dataArr = dataArr;
    
    NSString *danweishezhi =[[NSUserDefaults standardUserDefaults] objectForKey:@"danweishezhi"];
    if(danweishezhi !=NULL){
        MyTable.chooseContent=danweishezhi;
    }
    ...
    MyTable.block = ^(NSString *chooseContent,NSIndexPath *indexPath){
        ...
        [MyTable ReloadData];
        [[NSUserDefaults standardUserDefaults] setObject:chooseContent forKey:@"danweishezhi"];
        ...
        NSString *strcmd = [HexUtils getDanweiCmd:@"0001"];
        if(indexPath.row==0)
        {
            strcmd = [HexUtils getDanweiCmd:@"0001"];
        }
        else if(indexPath.row==1)
        {
            strcmd = [HexUtils getDanweiCmd:@"0002"];
        }
...
//        [cell addSubview:imageViewJian];
        ...
        labHeight  = [[UILabel alloc] initWithFrame:CGRectMake(self.view.frame.size.width-54, 0, self.view.frame.size.width-18, 51)];
        labHeight.tag = 122;
        labHeight.font = [UIFont systemFontOfSize:14];
        labHeight.backgroundColor = [UIColor clearColor];
        labHeight.textColor = [Common2 colorWithHexString:@"#31302f"];
        [cell addSubview:labHeight];
    ...
    labHeight = (UILabel*)[cell viewWithTag:122];
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            imageleft.image = [UIImage imageNamed:@"icon-order"];
             ...
             labHeight.text = [array objectAtIndex:indexPath.row];
        }
        else if (indexPath.row == 1) {
...
- (void)BLEManagerReceiveData:(NSData *)value fromPeripheral:(CBPeripheral *)peripheral andServiceUUID:(NSString *)serviceUUID andCharacteristicUUID:(NSString *)charUUID {
    ...
    if ([str_receve containsString:@"1B41"]) {
        if (str_receve.length < 18) {
            ...
            if(str_gaodu.length < 2 && ([str_gaodu isEqualToString:@"1"])) {
                [BLEManager sharedManager].is_CM = YES;
                MyTable.chooseContent= [NSString stringWithFormat:@"%@(cm)", LocationLanguage(@"cm", @"厘米")];
                [MyTable ReloadData];
            }else if ([str_gaodu isEqualToString:@"2"]) {
                [BLEManager sharedManager].is_CM = NO;
                MyTable.chooseContent=[NSString stringWithFormat:@"%@(inch)", LocationLanguage(@"inch", @"厘米")];
                [MyTable ReloadData];
            }
        }
    }
