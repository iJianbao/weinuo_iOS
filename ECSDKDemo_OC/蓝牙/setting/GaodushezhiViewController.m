...
#import "ZhanzimoshiViewController.h"
#import "JiyigaoduViewController.h"

#import "BLEManager.h"

@interface GaodushezhiViewController ()
...
@implementation GaodushezhiViewController
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 0) {
        if (indexPath.row == 0) {
            ...
            NSArray *zuoziArray = [str_zhanzigaodu componentsSeparatedByString:@"#"];
            NSString *showHeight = [BLEManager floatHeightNotLimit:[BLEManager autoInOrCm:[zuoziArray.firstObject floatValue]
                                                                             isIn:zuoziArray.lastObject
                                                                           needCm:[BLEManager sharedManager].is_CM]
                                                    needCm:[BLEManager sharedManager].is_CM];
            cell.detailTextLabel.text = [NSString stringWithFormat:@"%@%@", showHeight, [BLEManager sharedManager].is_CM ? @"cm" : @"IN"];
        }
        else if (indexPath.row == 1) {
            NSString *str_zuozigaodu = [BLEManager sharedManager].yy_zuoziInfo;
            NSArray *zhanziArray = [str_zuozigaodu componentsSeparatedByString:@"#"];
            NSString *showHeight = [BLEManager floatHeightNotLimit:[BLEManager autoInOrCm:[zhanziArray.firstObject floatValue]
        ...
        case 0:
            switch (indexPath.row) {
                ...
                case 0:
                {
                    ...
                    ZhanzimoshiViewController *videoRender = [[ZhanzimoshiViewController alloc] init];
                    videoRender.navigationController.navigationBarHidden = YES;
                    [self.navigationController pushViewController:videoRender animated:YES];
                ...
                case 1:
                {
                    ...
                    ZuozimoshiViewController *videoRender = [[ZuozimoshiViewController alloc] init];
                    videoRender.navigationController.navigationBarHidden = NO;
                    //                    videoRender.strName=_strtag;
                    [self.navigationController pushViewController:videoRender animated:YES];
                }
                ...
                case 2:
                {
                    JiyigaoduViewController *videoRender = [[JiyigaoduViewController alloc] init];
                    videoRender.navigationController.navigationBarHidden = YES;
                    [self.navigationController pushViewController:videoRender animated:YES];
                }
