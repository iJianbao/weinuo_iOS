...
//        }
//    }];
    [self.bleManager scanAndConnectDeviceWithName:@"BT05-A" callback:^(EasyPeripheral *peripheral, NSError *error) {
        if (!error) {
            weakself.peripheral = peripheral ;
        }
    }];
...
//    NSData *D = [[NSData alloc] initWithBytes:bytes length:sizeof(bytes)];
    ...
    NSData *D = [EasyUtils convertHexStrToData:number];
...
//        NSLog(@"%@ -- %@",data ,error );
//    }];
    [self.bleManager notifyDataWithPeripheral:self.peripheral serviceUUID:@"0000ffe0-0000-1000-8000-00805f9b34fb" notifyUUID:@"0000ffe1-0000-1000-8000-00805f9b34fb" notifyValue:YES withCallback:^(NSData *data, NSError *error) {
        NSLog(@"%@ -- %@",data ,error );
    }];
}
- (void)test
